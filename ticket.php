			   ...
			   <?php
					if($rparameters['ticket_type']==1)
					{
						...
						<div class="form-group row '; if((!$rright['ticket_type_disp'] && $_GET['action']!='new') || (!$rright['ticket_new_type_disp'] && $_GET['action']=='new')) {echo 'd-none';} echo'">
							...
							<div class="col-sm-5">
								<select id="type" name="type" class="form-control col-5" ';if($rparameters['user_validation'] && $rparameters['user_validation_perimeter']=='mark' && $rright['ticket_user_validation']) { echo ' onchange="loadVal(); submit();" ';} if($rright['ticket_type_mandatory']) { echo ' onchange="CheckMandatory();" ';} if(($rright['ticket_type']==0 && $_GET['action']!='new') || ($rright['ticket_new_type']==0 && $_GET['action']=='new')) {echo 'disabled="disabled"';} echo' >';
									//limit service type
				...
				<div class="form-group row <?php if(($rright['ticket_cat_disp']==0 && $_GET['action']!='new') || ($rright['ticket_new_cat_disp']==0 && $_GET['action']=='new')) echo 'd-none';?>">
					...
					<div class="col-sm-5">
						...
						<select <?php if($mobile) {echo 'style="max-width:105px;"';}else{echo 'style="width:auto;"';}?>  class="form-control d-inline-block mb-1 mb-md-0" title="<?php echo T_('Catégorie'); ?>" id="category" name="category" <?php if(($rright['ticket_cat']==0 && $_GET['action']!='new') || ($rright['ticket_new_cat']==0 && $_GET['action']=='new')) echo ' disabled="disabled" ';?>>
						<?php
							//if user limit service restrict category to associated service
							if($rparameters['user_limit_service'] && $rright['dashboard_service_only']!=0) //case for user who open ticket to auto-select categories of the service
							{
				...
				<div class="form-group row <?php if(!$rparameters['user_validation'] || $rparameters['user_validation_perimeter']=='all' || !$rright['ticket_user_validation'] || $_GET['action']=='new') {echo 'd-none';}?>">
					<div class="col-sm-2 col-form-label text-sm-right pr-0">
						<label class="mb-0" for="user_validation"><?php echo T_('Validation demandeur'); ?> :</label>
					</div>
					<div class="col-sm-5">
						<label>
							<input type="radio" class="ace mt-2" name="user_validation" value="1" <?php if($globalrow['user_validation']) echo 'checked'; ?>> <span class="lbl"> Oui </span>
							<input type="radio" class="ace mt-2" name="user_validation" value="0" <?php if(!$globalrow['user_validation']) echo 'checked'; ?>> <span class="lbl"> Non </span>
						</label>
						<?php
							//display validation date field
							if(($globalrow['user_validation'] || $_POST['user_validation']) && $globalrow['state']==3)
							{
				...
				<div class="border-t-1 brc-secondary-l1 bgc-secondary-l3 py-3 text-center">
					...
					if(!$hide_button)
					{
						...
						if($rright['ticket_close'] && $_POST['state']!='3' && $globalrow['state']!='3' && $_GET['action']!='new' && $lock_tech==0)
						{
							echo '
	...
	jQuery(function($) {
		//get and display user informations
			...
			function GetUserInfos(user) {
				...
				$.ajax({ 
					type: "POST", 
					url: "ajax/ticket_userinfos.php?token=<?php echo $_COOKIE["token"]; ?>&ticket=<?php echo $_GET['id']; ?>", 
					data: dataString, 
					success: function(result){
						...
						if(data.custom1) {$("#user_custom1").html('&nbsp;&nbsp;&nbsp;<a href=""><i class="fa fa-info-circle text-info"></i></a> '+data.custom1);} else {$("#user_custom1").html('');}
						if(data.custom2) {$("#user_custom2").html('&nbsp;&nbsp;&nbsp;<a href=""><i class="fa fa-info-circle text-info"></i></a> '+data.custom2);} else {$("#user_custom2").html('');}
